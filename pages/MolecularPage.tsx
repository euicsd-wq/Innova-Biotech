...
const FeaturedProductCard: React.FC<{ test: MolecularTestCategory['tests'][0] }> = ({ test }) => (
    <ReactRouterDOM.Link to={`/products/molecular/${test.slug}`} className="bg-white rounded-2xl shadow-lg ring-1 ring-border-color hover:ring-brand-secondary/70 hover:shadow-2xl transition-all duration-300 flex flex-col md:flex-row overflow-hidden group">
...
const ProductCard: React.FC<{ test: MolecularTestCategory['tests'][0] }> = ({ test }) => (
    <ReactRouterDOM.Link to={`/products/molecular/${test.slug}`} className="bg-white p-6 rounded-xl shadow-md ring-1 ring-border-color flex flex-col hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 group border-t-4 border-brand-light-bg hover:border-brand-secondary">
    ...
    return (
        <div className="bg-brand-light-bg">
            <PageHero
                ...
                subtitle="High-performance molecular solutions for the sensitive and specific detection of pathogens and genetic markers, providing definitive answers for critical diagnostic challenges."
